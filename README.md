...
Hands-on Microsoft Entra ID lab implementing and troubleshooting SAML 2.0 Single Sign-On (SSO) with **Microsoft Entra ID** as the Identity Provider (IdP) and the **Microsoft Entra SAML Toolkit** as the Service Provider (SP).

The goal was not only to make SSO work, but to understand the complete authentication and authorization flow, and to troubleshoot realistic identity-access failures.
...
A successful Microsoft Entra authentication does **not** automatically mean the user receives application access. The flow involves multiple layers:

1. **Authentication** – Entra verifies who the user is.
2. **Conditional Access / MFA** – Entra evaluates authentication requirements.
3. **Enterprise Application authorization** – Entra verifies whether the user or their group is allowed to use the application.
4. **SAML assertion** – Entra creates and signs an assertion containing identity claims.
5. **Service Provider validation** – The application validates the assertion.
6. **Application account matching/provisioning** – The application must know how to map the federated identity to an application account.
7. **Application session** – The application finally grants access.
...
- The SAML Toolkit acted as the **Service Provider (SP)**
...
The ACS endpoint is where Microsoft Entra ID sends the SAML response after authentication. Example structure:
...
Microsoft Entra ID signs the SAML assertion using its SAML signing certificate. The Service Provider uses the corresponding certificate information to verify that:
...
with the Name identifier format: **Email address**
...
Instead of assigning every user individually:
...
Adding Sara to the authorized group allowed Entra to authorize her for the Enterprise Application. This demonstrated the distinction between:

- **Authentication** — Is this really Sara?
- **Authorization** — Is Sara allowed to access this application?

## 8. SP-Initiated SSO Flow

The lab tested Service Provider–initiated SSO. The complete flow was:
...
Successful SSO was tested with authorized users. The browser was redirected through Microsoft Entra authentication and returned to the SAML Toolkit.

Microsoft Entra sign-in logs showed successful authentication events for the Enterprise Application, verifying that:
...
**Investigation**
The Enterprise Application sign-in logs were reviewed. The user successfully reached Microsoft Entra ID, but Entra rejected application access because the user was not directly assigned and was not a member of an authorized group.
...
**Resolution**
The user was added to the appropriate security group. After the membership change, Entra allowed the SAML authentication flow to continue.
...
**Investigation**
The SAML assertion was successfully generated and sent to the Service Provider — proving that Entra authentication and authorization were working. The issue occurred after the assertion reached the application.

**Root Cause**
The SAML Toolkit is a test application that requires a matching local user account. The Service Provider could not map the incoming NameID to an existing local application user.

**Resolution**
A matching local user was created in the SAML Toolkit. SSO then succeeded.
...
**Investigation**
The Attributes & Claims configuration was reviewed. The Service Provider expected the NameID in email-address format:
...
**Investigation**
The ACS URL generated by the Service Provider was compared with the Reply URL configured in Microsoft Entra ID. The values did not match.
...
**Lesson**
The Reply URL is not simply the application's homepage — it is the specific endpoint where the Service Provider is waiting to receive and process the SAML response.

## Sign-In Log Investigation

Microsoft Entra sign-in logs were used throughout the troubleshooting process. The logs helped identify:
...
The IdP authenticates the identity. The SP provides the application or service.
...
In a production environment, the two technologies work together:
...
Screenshots collected during the lab can be stored in `screenshots/`.
...
The biggest lesson from this lab was that enterprise SSO is more than entering a username and password. A working application login can depend on the entire chain:
